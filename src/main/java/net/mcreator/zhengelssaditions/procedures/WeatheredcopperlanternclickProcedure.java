...
public class WeatheredcopperlanternclickProcedure {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() instanceof AxeItem) {
			{
				...
				BlockState _bs = ZhengelssAditionsModBlocks.EXPOSEDCOPPERLANTERN.get().defaultBlockState();
			...
			{
				BlockPos _bp = BlockPos.containing(x, y, z);
				BlockState _bs = ZhengelssAditionsModBlocks.WAXEDWEATHEREDCOPPERLANTERN.get().defaultBlockState();
				BlockState _bso = world.getBlockState(_bp);
				for (Map.Entry<Property<?>, Comparable<?>> entry : _bso.getValues().entrySet()) {
					Property _property = _bs.getBlock().getStateDefinition().getProperty(entry.getKey().getName());
					if (_property != null && _bs.getValue(_property) != null)
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles(ParticleTypes.WAX_ON, (x + 0.5), y, (z + 0.5), 5, 0.5, 0.5, 0.5, 0);
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
